...
export const ko: Strings = {
	...
	start: {
		...
		bridgeSteps: `# 1 · 호스트에서 taixu-coding-bridge 다운로드 & 압축 해제(runtime과 동일 릴리스)
# 2 · 토큰을 설정해 실행(호스트에 claude / codex CLI 필요)
CODINGBRIDGE_TOKEN=your-secret ./taixu-coding-bridge      # 127.0.0.1:8765 에서 수신
# 3 · 패널의 "코딩 브리지"에 URL(http://host.docker.internal:8765) + 동일 토큰 입력`,
